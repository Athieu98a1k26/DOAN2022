...
import api from "../../api/phongbanapi";
...
const App = () => {
    ...
    const handleOk = (edit) => {
        // onFinish()
        if (edit) {
            api.suaphongban({ ...state, bienChe: true }).then(d => {
                console.log(d, "đ")
            ...
            api.sendphongban({ tenPhongBan: state.tenPhongBan, bienChe: true }).then(d => {
                console.log(d, "đ")
    ...
    return (
        <div className='content' style={{ marginTop: 50 }}>
            ...
            <Modal
                key={state?.id}
                title={!state.edit ? "Thêm phòng ban" : "Sửa phòng ban"}
                ...
                >
                    ...
                    >
                        <Input defaultValue={state?.tenPhongBan} name="tenPhongBan" onChange={onChange} />
                    </Form.Item>
                    <Form.Item
                        label="Ghi chú"
                        name="ghichu"
                        rules={[{ required: true, message: 'Nhập tên ghi chú' }]}
                    >
