...
const App = () => {
    ...
    const handleOk = (edit) => {
        // onFinish()
        if (edit) {
            api.suadonvi({ ...state, bienChe: true }).then(d => {
                console.log(d, "đ")
            ...
            api.senddonvi({ tenDonVi: state.tenDonVi, bienChe: true }).then(d => {
                console.log(d, "đ")
    ...
    return (
        <div className='content' style={{ marginTop: 50 }}>
            ...
            <Modal
                key={state?.id}
                title={state?.edit ? "Thêm đơn vị" : "Sửa đơn vị"}
                ...
                >
                    ...
                    >
                        <Input defaultValue={state?.tenDonVi} name="tenDonVi" onChange={onChange} />
                    </Form.Item>
                </Form>
            </Modal>
        </div>
