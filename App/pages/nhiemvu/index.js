...
import api from "../../api/nhiemvuapi";
...
const App = () => {
    ...
    const handleOk = (edit) => {
        // onFinish()
        if (edit) {
            api.suanhiemvu({ ...state, bienChe: true }).then(d => {
                console.log(d, "đ")
            ...
            api.sendnhiemvu({ tenNhiemVu: state.tenNhiemVu, bienChe: true }).then(d => {
                console.log(d, "đ")
    ...
    return (
        <div className='content' style={{ marginTop: 50 }}>
            ...
            <Modal
                key={state?.id}
                title={!state.edit ? "Thêm học hàm" : "Sửa học hàm"}
                ...
                >
                    <Form.Item
                        label="Tên học hàm"
                        name="ten"
                        rules={[{ required: true, message: 'Nhập tên học hàm!' }]}
                    >
                        <Input defaultValue={state?.tenNhiemVu} name="tenNhiemVu" onChange={onChange} />
                    </Form.Item>
                </Form>
            </Modal>
        </div>
