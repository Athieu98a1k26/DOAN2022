import React, { useState, useEffect } from 'react';
import { Modal, Form, Input, Button, Checkbox, DatePicker, message, List, Table, Divider } from 'antd';
import api from "../../api/chuyenmonapi";
import moment from 'moment';
import DatePicker1 from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
const App = () => {
    const [isModalVisible, setIsModalVisible] = useState(false);

    const showModal = () => {
        setState({ ...state, edit: false })
        setIsModalVisible(true);
    };

    useEffect(() => {
        api.getDetail().then(d => {
            setState({ ...state, data: d.list, total: d.total })
        })
    }, []);

    function onChangeSearch(e) {
        const { value, name } = e.target;
        api.getDetail({ search: value }).then(d => {
            setState({ ...state, data: d.list, total: d.total })
        })
    }

    const handleOk = (edit) => {
        // onFinish()
        if (edit) {
            api.suachuyenmon({ ...state, bienChe: true }).then(d => {
                console.log(d, "đ")
                message.success("Sửa thành công")
                setIsModalVisible(false);
                api.getDetail().then(dd => {
                    setState({ ...state, data: dd.list, total: dd.total, edit: false })
                })

            }).catch(e => {
                console.log(e, "")
            })
        } else {
            api.sendchuyenmon({ hocHam: state.hocHam, chuyenNganhTN: state.chuyenNganhTN, namTN: state.namTN, noiTN: state.noiTN, bienChe: true }).then(d => {
                console.log(d, "đ")
                message.success("Thêm thành công")
                setIsModalVisible(false);
                api.getDetail().then(dd => {
                    setState({ ...state, data: dd.list, total: dd.total, edit: false })
                })

            }).catch(e => {
                console.log(e, "")
            })
        }
    };

    const handleCancel = () => {
        setIsModalVisible(false);
    };

    const remove = (id) => {
        api.xoachuyenmon({ id }).then(d => {
            console.log(d, "đ")
            message.success("Xóa thành công")
            api.getDetail().then(d => {
                setState({ ...state, data: d.list, total: d.total, edit: false })
            })
        }).catch(e => {
            console.log(e, "")
        })
    }

    const edit = (id) => {
        setState({ ...state, edit: true })
        api.getId({ id }).then(d => {
            if (d) {
                console.log(d, "ff")
                setState({ ...state, ...d, edit: true })

            }
            setIsModalVisible(true);
        }).catch(e => {
            console.log(e, "")
        })
    }

    const onFinish = (values) => {
        console.log('Success:', values);
    };

    const onFinishFailed = (errorInfo) => {
        console.log('Failed:', errorInfo);
    };

    function onChange(e) {
        const { value, name } = e.target;
        setState({
            ...state,
            [name]: value,
        });
        console.log(`value = ${value}`);
        console.log(`name = ${name}`);
    }
    const [state, setState] = useState({
        data: [],
        currentData: null,
        edit: false
    });
    const [date, setDate] = useState(null);
    const handleChange = (value, name) => {
        console.log(value, "datae", name)
        // message.info(`Selected Date: ${value ? value.format('YYYY-MM-DD') : 'None'}`);
        // setDate(value);
        // if(name == "namHoc"){
        setState({ [name]: value })
        // }
    };
    const [namTN, setnamTN] = useState(new Date());
    const handleChangenamhoc = (value, name) => {
        console.log(value, "value------")
        setState({ ...state, namTN: moment(value).toISOString() })
        setnamTN(value)
    };

    const columns = [
        {
            title: 'Chuyên ngành TN',
            dataIndex: 'hoten',
            key: 'hoten',
            // render: text => <a>{text}</a>,
        },
        {
            title: 'Học hàm',
            dataIndex: 'hocham',
            key: 'hocham',
        },
        {
            title: 'Năm TN',
            dataIndex: 'namtn',
            key: 'namtn',
        },
        {
            title: 'Nơi TN',
            dataIndex: 'noitn',
            key: 'noitn',
        },
        {
            title: 'Action',
            dataIndex: 'Action',
            key: 'Action',
            // render: text => <a>{text}</a>,
            render: (text, record) => (
                <>
                    <button onClick={() => edit(record.id)}>
                        {"Sửa"}
                    </button>&nbsp;
                    <button onClick={() => remove(record.id)}>
                        {"Xóa"}
                    </button>
                </>
            ),
        },

    ];

    var data1 = []
    if (state.data) {
        data1 = state.data.map(x => (
            x
        ))
    }

    const data = state.data.map((x, k) =>
    ({
        key: k + "",
        hoten: x.chuyenNganhTN,
        hocham: x.hocHam,
        namtn: moment(x.namTN).format("YYYY"),
        noitn: x.noiTN,
        id: x.id
    })
    )


    return (
        <div className='content' style={{ marginTop: 50 }}>
            <div style={{ display: "flex", justifyContent: "right", marginBottom: 20 }}>
                <Input onChange={onChangeSearch} placeholder="Search" />
                <Button type="primary" onClick={showModal} >
                    Thêm
                </Button>
            </div>
            <Table columns={columns} dataSource={data} />

            <Modal
                key={state?.id}
                title={state?.edit ? "Thêm chuyên môn" : "Sửa chuyên môn"}
                visible={isModalVisible}
                onOk={() => handleOk(state.edit)}
                onCancel={handleCancel}>
                <Form
                    name="basic"
                    labelCol={{ span: 8 }}
                    wrapperCol={{ span: 16 }}
                    // initialValues={{ remember: true }}
                    onFinish={onFinish}
                    onFinishFailed={onFinishFailed}
                    autoComplete="off"
                >
                    <Form.Item
                        label="Học hàm"
                        name="ten"
                        rules={[{ required: true, message: 'Nhập học hàm!' }]}
                    >
                        <Input defaultValue={state?.hocHam} name="hocHam" onChange={onChange} />
                    </Form.Item>
                    <Form.Item
                        label="Chuyên ngành"
                        name="chuyennganh"
                        rules={[{ required: true, message: 'Nhập chuyên ngành' }]}
                    >
                        <Input defaultValue={state?.chuyenNganhTN} name="chuyenNganhTN" onChange={onChange} />
                    </Form.Item>
                    <Form.Item
                        label="Năm TN"
                        name="namtn"
                        rules={[{ required: true, message: 'Nhập năm TN' }]}
                    >
                        <DatePicker1
                            dateFormat="yyyy"
                            selected={namTN}
                            onChange={(date, dateString) => handleChangenamhoc(date, dateString, 1)}
                        />
                    </Form.Item>
                    <Form.Item
                        label="Nơi TN"
                        name="noitn"
                        rules={[{ required: true, message: 'Nhập nơi TN' }]}
                    >
                        <Input defaultValue={state?.noiTN} name="noiTN" onChange={onChange} />
                    </Form.Item>
                </Form>
            </Modal>
        </div>
    );
};
export default App;