    ...
    if (localStorage.getItem("breakCancel") === "on") {
      ...
      if (codeErr === "14485") {
        ...
        if (+receivedAmount > +amount) {
          ...
          if (+receivedAmount !== 0) {
            let arr = arrTransactionDetail.concat(
              formatTransactionDetail(data)
            );
            arrTransactionDetail = arrTransactionDetail.concat(
              formatTransactionDetail(data)
            );

            const setjson22 = JSON.stringify(arr);
            localStorage.setItem("arrTransactionDetail", setjson22);
          }
